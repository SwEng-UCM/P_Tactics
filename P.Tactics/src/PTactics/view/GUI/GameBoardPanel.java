	...
	public GameBoardPanel(int width, int height, ControllerInterface cntr, ControlPanel cPanel) {
		...
		_CPUPanel = new JPanel(new BorderLayout()) {
			...
			protected void paintComponent(Graphics g) {
				...
				g.drawImage(Icons.otherIcons.LABELBACKGROUND.getImage(), 0, 0, getWidth(), width * Position.tileSize,
						this);
			}
		};
		...
		for (int row = 0; row < width; row++) {
			for (int col = 0; col < height; col++) {
				...
				btn.addActionListener(e -> {
					// DEBUG: System.out.println("GameWidth: " + PTactics.utils.Position._gameWidth
					// + " Cell " + btn.getPosition().getX() + ", " + btn.getPosition().getY());
					if (this._cntr.isTroop(btn.getPosition())) {
						String[] cmdArgs = { Utils.CommandInfo.COMMAND_SELECT_NAME,
								Integer.toString(btn.getPosition().getY() + 1),
								Integer.toString(btn.getPosition().getX() + 1) };
						// + 1 because parser assumes pos + 1
						_cntr.executeCommand(cmdArgs);
						/*
						 * SelectTroopCommand select= new
						 * SelectTroopCommand(btn.getPosition().getX(),btn.getPosition().getY());
						 * select.execute(cntr);
						 */
					} else if (this._cPanel.getControlSelection() == 0) {
						String[] cmdArgs = { Utils.CommandInfo.COMMAND_MOVE_NAME,
								Integer.toString(btn.getPosition().getY() + 1),
								Integer.toString(btn.getPosition().getX() + 1) };
						// + 1 because parser assumes pos + 1
						_cntr.executeCommand(cmdArgs);
						// MoveCommand move = new
						// MoveCommand(btn.getPosition().getX(),btn.getPosition().getY());
						// move.execute(cntr);
						// _cPanel.resetControlSelection();
					} else if (this._cPanel.getControlSelection() == 1) {
						String[] cmdArgs = { Utils.CommandInfo.COMMAND_AIM_NAME,
								posToDir(btn.getPosition().getX(), btn.getPosition().getY()).toString() };
						_cntr.executeCommand(cmdArgs);
						// AimCommand aim = new AimCommand(posToDir(btn.getPosition().getX(),
						// btn.getPosition().getY()));
						// aim.execute(cntr);
						// _cPanel.resetControlSelection();

					} else if (this._cPanel.getControlSelection() == 2) {
						String[] cmdArgs = { Utils.CommandInfo.COMMAND_ABILITY_NAME,
								Integer.toString(btn.getPosition().getY() + 1),
								Integer.toString(btn.getPosition().getX() + 1) };
						// + 1 because parser assumes pos + 1
						_cntr.executeCommand(cmdArgs);
						// AbilityCommand ability= new
						// AbilityCommand(btn.getPosition().getX(),btn.getPosition().getY());
