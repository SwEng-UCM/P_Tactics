...
public class SelectTroopCommand extends Command {
	...
	public void execute(ControllerInterface CI, Troop t) {	
		...
		if(!pos.isValid()) throw new Exception(Utils.MsgErrors.INVALID_COORDINATES);
		...
		if (!g.isAlive()) {
			throw new Exception(Utils.MsgErrors.INVALID_SELECTION);		//Have to check if it is a troop alive (walls and dead troops will return false)
		}
		if(!g.getPlayer().equals(i.toString())) {
			throw new Exception(Utils.MsgErrors.INVALID_SELECTION);   		//Have to check that it belongs to the player (sorry for the casting)
		}
		CI.setTroop(g);
